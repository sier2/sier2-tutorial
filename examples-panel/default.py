...
class Data(Block):
    ...
    extra = param.String(doc='An extra variable only used by this block')
...
def make_dag():
    ...
    dag = PanelDag(
        [
            (data1.param.out_nick, data2.param.in_nick),
            (data1.param.out_age, data2.param.in_age),
            (data1.param.out_birth, data2.param.in_birth),
            (data2.param.out_nick, data3.param.in_nick),
            (data2.param.out_age, data3.param.in_age),
            (data2.param.out_birth, data3.param.in_birth),
        ],
        title='Default panels',
        doc='Demonstrate using default panels\nThis block demonstrates using a custom panel incorporating the default panel. We allow the `PanelDag` to generate the default widgets, and use the result to build our own custom GUI, adding instructions alongside the displayed fields.',
    )
