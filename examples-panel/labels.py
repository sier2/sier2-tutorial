...
class Inputs(Block):
    ...
    in_age_in_years_label = param.Integer(
        doc='A labelled integer number of years',
        bounds=(0, 100),
        label='A custom label; do not alter',
    )
    in_full_name = param.String(doc='Your full name')
    in_australian_citizen = param.Boolean(doc='Are you an Australian citizen?')
    in_australian_domicile_labelled = param.Boolean(
        doc='Are you living in Australia?', label='In Australia? (custom label starting with "In ")'
    )
    in_favourite_country = param.Selector(
        objects=['Australia', 'Canada', 'Great Britain', 'New Zealand', 'United States of America'],
        doc='Pick a country',
    )
    ...
    def __init__(self):
        super().__init__(name='params with labels', wait_for_input=True, continue_label='Execute')
